function hexToVec3(hex) {
  ...
  function initFloatingLines(canvasElement, options = {}) {
    const {
        linesGradient = ["#050810", "#1a2a6c", "#4facfe", "#00f0ff"], // specific Nocturne colors in static site
  ...
  void main() {
    ...
    const handlePointerMove = event => {
        ...
        targetMouse.set(x * dpr, (height - y) * dpr);
        targetInfluence = 1.0;
  ...
  // Automatically bind if #bg-canvas exists
  document.addEventListener('DOMContentLoaded', () => {
    const canvas = document.getElementById('bg-canvas');
    if (canvas) {
      initFloatingLines(canvas, {
         linesGradient: ['#050810', '#1a2a6c', '#4A90E2', '#00F5FF'], 
         mixBlendMode: 'screen',
         lineCount: [5, 5, 5],
         lineDistance: [5, 5, 5],
         bendRadius: 5,
